...
var fetchQuery = function(collection, role, fielder, opts, callback) {
    return function(query) {
        authenticate(role);

        console.log(fielder, role, opts);
        var field = fielder || 'name';
        ...
        var options = _.extend(options, opts);
        console.log(options);
...
Meteor.methods({
    'clients': fetchQuery(Meteor.users, 'view-clients', 'profile.name', {type: 'client'}),
    'lawyers': fetchQuery(Meteor.users, 'view-lawyers', 'profile.name', {type: 'lawyer'}),
    'courts': fetchQuery(Courts, 'view-courts'),
    'cases': fetchQuery(Projects, 'view-projects'),
    'labels': fetchQuery(Labels, 'view-labels'),
    'tasks': fetchQuery(Tasks, 'view-tasks'),
