projectSchema = new SimpleSchema({
  ...
  type: {
    ...
    optional: true,
    label: 'Type of case',
    allowedValues: ['other','business', 'criminal', 'civil', 'commercial', 'corporate', 'family', 'immigration', 'insurance', 'personalinjury', 'tax']
  ...
  clientIds: {
    ...
    autoform: {
      type: "selectize",
      options: function(){
        return getUserOptions({'type': 'client'}, {'labelKey': 'name', 'valueKey': '_id'}); 
      },
      afFieldInput: {
        multiple: true
      }
  ...
  "reminders.$.email": {
    ...
    autoform: {
      type: "selectize",
      options: function(){
        return getUserOptions({}, {'labelKey': 'name', 'valueKey': 'email'});
      }
    }
  },
  hearingIds : {
...
Projects.helpers({
  ...
  subject: function(){
    return sprintf('[%s] reminder for matter: %s', this._id, this.name);
  },
  lawyers: function(){
    // TODO: Test performance here
    return Meteor.users.find({_id: {$in:this.lawyerIds}}, {transform: transformUserForMandrill}).fetch();
  },
  clients: function(){
    // TODO: Test performance here
    return Meteor.users.find({_id: {$in:this.clientIds}}, {transform: transformUserForMandrill}).fetch();
  },
...
EasySearch.createSearchIndex('projects', {
  ...
  // 'use' : 'mongo-db', Remove this cause it breaks publications
  'props': {
    'lawyers': false,
    'clients': false,
    'courtId': false,
  ...
  'query': function(searchString, opts) {
    ...
    if(this.props.lawyers)
      query.lawyerIds = {$all: this.props.lawyers};
    if(this.props.clients)
      query.clientIds = {$all: this.props.clients};
    if(this.props.type)
      query.type = this.props.type;
    if(this.props.courtId)
      query.courtId = this.props.courtId;
    if(this.props.labels)
      query.labelIds = {$all: this.props.labels};
